...
exports.memberAffairsService = exports.MemberAffairsService = void 0;
...
class MemberAffairsService {
    ...
    async approveMember(adminId, userId, preferredClassId) {
        ...
        if (!finalClassId) {
            ...
            if (!defaultClass)
                throw new errors_1.BadRequestError('No default service class available');
        ...
        try {
            ...
            if (assignedClass && assignedClass.users.length > 0) {
                ...
                await notifications_repository_1.notificationsRepository.spawnBulkNotifications(managerIds, {
                    actorID: adminId,
                    type: 'MEMBERSHIP',
    ...
    async rejectMember(userId, reason) {
        const updated = await prisma.user.update({
            where: { id: userId },
    ...
    // -------------------- MEMBER CENSUS --------------------
    async listMembers(filters, accessLevel, userServiceClassId) {
        const where = { system_role: 'MEMBER' };
        ...
        if (filters.department)
            where.academic_dept = filters.department;
        if (filters.search) {
            where.OR = [
    ...
    async updateMember(adminId, memberId, data) {
        ...
        if (data.repentance_father_id !== undefined)
            updateData.repentance_father_id = data.repentance_father_id || null;
        if (data.repentance_deacon_id !== undefined)
            updateData.repentance_deacon_id = data.repentance_deacon_id || null;
        if (data.spiritual_father_id !== undefined)
            updateData.spiritual_father_id = data.spiritual_father_id || null;
        if (data.spiritual_mother_id !== undefined)
            updateData.spiritual_mother_id = data.spiritual_mother_id || null;
        if (data.is_active !== undefined)
            updateData.is_active = data.is_active;
        if (data.academic_year !== undefined)
            updateData.academic_year = data.academic_year ? parseInt(data.academic_year, 10) : null;
        if (data.academic_dept !== undefined)
            updateData.academic_dept = data.academic_dept || null;
        if (data.phone_number !== undefined)
            updateData.phone_number = data.phone_number || null;
        if (data.dorm_block !== undefined)
            updateData.dorm_block = data.dorm_block || null;
        if (data.dorm_room !== undefined)
            updateData.dorm_room = data.dorm_room || null;
        const updated = await prisma.user.update({
            where: { id: memberId },
            data: updateData,
        });
    ...
    async getSpiritualCandidates(role) {
        if (role === 'priest') {
    ...
    async assignSpiritual(memberId, role, valueId) {
        const updateData = {};
        if (role === 'repentance_father_id')
            updateData.repentance_father_id = valueId;
        else if (role === 'repentance_deacon_id')
            updateData.repentance_deacon_id = valueId;
        else if (role === 'spiritual_father_id')
            updateData.spiritual_father_id = valueId;
        else if (role === 'spiritual_mother_id')
            updateData.spiritual_mother_id = valueId;
        else
            throw new errors_1.BadRequestError('Invalid role');
        return prisma.user.update({ where: { id: memberId }, data: updateData });
    }
    // -------------------- BATCH ASSIGNMENT --------------------
    async batchAssignClass(memberIds, serviceClassId) {
    ...
    async listSubClasses(serviceClassId) {
    ...
    async hasSufficientGraduation(userId) {
        ...
        let phases = [];
        try {
            phases = JSON.parse(user.graduated_phases);
    ...
    async validateLeaderGraduation(leaderId, roleName) {
        const hasGraduation = await this.hasSufficientGraduation(leaderId);
        if (!hasGraduation) {
            throw new errors_1.BadRequestError(`${roleName} must have completed at least Gubae Hawaryat (or higher) to be assigned as sub‑class leader.`);
        }
    }
    async createSubClass(serviceClassId, data, requestedById, bypassApproval = false) {
        const { sub_class_name, sub_chair_id, sub_secretary_id, sub_vice_id } = data;
        if (!sub_class_name?.trim())
            throw new errors_1.BadRequestError('sub_class_name is required');
        ...
        if (existingSubClass)
            throw new errors_1.BadRequestError(`Sub-class "${sub_class_name}" already exists in this service class`);
        ...
        if (leaderIds.length > 0) {
            ...
            if (existingLeadership)
                throw new errors_1.BadRequestError('One or more selected leaders already hold leadership positions in other sub‑classes of this service class');
        }
        ...
        for (const leaderId of leaderIds) {
            ...
            if (!leader || leader.service_class_id !== serviceClassId) {
                throw new errors_1.BadRequestError(`Selected leader must be a member of ${serviceClass.class_name_amharic}`);
            }
        }
        if (sub_chair_id)
            await this.validateLeaderGraduation(sub_chair_id, 'Sub‑chair');
        if (sub_secretary_id)
            await this.validateLeaderGraduation(sub_secretary_id, 'Sub‑secretary');
        if (sub_vice_id)
            await this.validateLeaderGraduation(sub_vice_id, 'Sub‑vice');
        const createData = {
            parent_class_id: serviceClassId,
        ...
        if (!bypassApproval) {
            await approval_service_1.approvalService.requestSubClassApproval('CREATE', subClass.id, requestedById, data);
            return { ...subClass, _notice: 'Sub-class created with PENDING_APPROVAL status. Chairman notification sent.' };
        }
        return subClass;
    }
    async addMemberToSubClass(subClassId, userId) {
        return prisma.subClassMember.create({ data: { sub_class_id: subClassId, user_id: userId } });
    }
    async updateSubClass(serviceClassId, subClassId, data) {
        ...
        if (data.sub_class_name !== undefined && data.sub_class_name.trim() !== existing.sub_class_name) {
            ...
            if (duplicate)
                throw new errors_1.BadRequestError(`Sub-class "${data.sub_class_name}" already exists in this service class`);
        ...
        if (data.sub_chair_id !== undefined)
            updateData.sub_chair_id = data.sub_chair_id || null;
        if (data.sub_secretary_id !== undefined)
            updateData.sub_secretary_id = data.sub_secretary_id || null;
        if (data.sub_vice_id !== undefined)
            updateData.sub_vice_id = data.sub_vice_id || null;
    ...
    async removeMemberFromSubClass(subClassId, userId) {
        return prisma.subClassMember.deleteMany({ where: { sub_class_id: subClassId, user_id: userId } });
    }
    async deleteSubClass(subClassId, userId) {
        if (!subClassId)
            throw new errors_1.BadRequestError('Sub-class ID is required');
        ...
        if (!isSecretariat && !isServiceManagerOfClass)
            throw new errors_1.ForbiddenError('You do not have permission to delete this sub‑class');
        if (subClass.members.length > 0)
            throw new errors_1.BadRequestError('Cannot delete sub‑class that has members. Remove members first.');
        return prisma.sub_classes.delete({ where: { id: subClassId } });
    }
    // -------------------- DOCUMENTS --------------------
    async uploadDocument(serviceClassId, data, uploadedBy, userRole) {
        ...
        if (serviceClassId && serviceClassId !== '') {
            ...
            throw new errors_1.BadRequestError('Service class ID is required for non-secretariat users');
        }
    ...
    async listDocuments(serviceClassId, type, userId, userRole) {
    ...
    async getDocumentById(documentId, userId, userRole) {
        ...
        if (!isChairman && !isUploader && doc.status !== 'APPROVED') {
            throw new errors_1.ForbiddenError('You do not have permission to view this document');
        }
        return doc;
    }
    async approveDocument(documentId, approvedBy) {
        ...
        if (doc.status !== 'PENDING')
            throw new errors_1.BadRequestError('Document is not pending approval');
    ...
    async rejectDocument(documentId, approvedBy, reason) {
    ...
    // ============ COMMENTS ============
    async addComment(documentId, userId, content, parentId) {
        const doc = await prisma.departmentDocument.findUnique({ where: { id: documentId } });
        if (!doc)
            throw new errors_1.NotFoundError('Document not found');
        const user = await prisma.user.findUnique({ where: { id: userId } });
        ...
        if (doc.status !== 'APPROVED' && !isChairman && !isUploader) {
            throw new errors_1.ForbiddenError('Cannot comment on this document');
        }
    ...
    // ============ REACTIONS ============
    async addReaction(documentId, userId, reactionType) {
        const doc = await prisma.departmentDocument.findUnique({ where: { id: documentId } });
        if (!doc)
            throw new errors_1.NotFoundError('Document not found');
        const user = await prisma.user.findUnique({ where: { id: userId } });
        ...
        const isUploader = doc.uploaded_by === userId;
        if (doc.status !== 'APPROVED' && !isChairman && !isUploader) {
            throw new errors_1.ForbiddenError('Cannot react to this document');
        }
    ...
    async removeReaction(documentId, userId) {
        return prisma.documentReaction.delete({
            where: { document_id_user_id: { document_id: documentId, user_id: userId } },
        });
    }
    // ============ NOTIFICATION HELPERS ============
    async notifyDocumentApproved(documentId, excludeUserId) {
        const doc = await prisma.departmentDocument.findUnique({
            where: { id: documentId },
            include: { uploader: { select: { full_name_three_parts: true } } },
        });
        ...
        if (uniqueRecipients.length === 0)
            return;
        await prisma.$transaction(uniqueRecipients.map(userId => prisma.notification.create({
            data: {
                user_id: userId,
    ...
    async notifyChairmanOfPendingDocument(uploadedById) {
        try {
    ...
    async notifyDocumentRejected(userId, documentTitle, reason) {
        try {
            await prisma.notification.create({
                data: {
                    user_id: userId,
    ...
    async notifyCommentAdded(userId, documentTitle) {
        try {
            await prisma.notification.create({
                data: {
                    user_id: userId,
    ...
    async notifyReactionAdded(userId, documentTitle) {
        try {
            await prisma.notification.create({
                data: {
                    user_id: userId,
    ...
    // ============ UPDATE DOCUMENT ============
    async updateDocument(documentId, userId, userRole, data) {
        const doc = await prisma.departmentDocument.findUnique({ where: { id: documentId } });
        if (!doc)
            throw new errors_1.NotFoundError('Document not found');
        const isChairman = userRole === 'SECRETARIAT_CHAIRMAN';
        ...
        if (!isChairman && !isUploader) {
            throw new errors_1.ForbiddenError('You do not have permission to edit this document');
        }
        // Uploader can only edit title, description, drive_url – NOT status
        if (!isChairman && (data.status !== undefined)) {
            throw new errors_1.ForbiddenError('Only the chairman can change document status');
        }
        const updateData = {};
    ...
    async deleteDocument(documentId, userId, userRole) {
        ...
        const isUploader = doc.uploaded_by === userId;
        if (!isChairman && !isUploader) {
            throw new errors_1.ForbiddenError('You do not have permission to delete this document');
        }
        return prisma.departmentDocument.delete({ where: { id: documentId } });
    }
}
exports.MemberAffairsService = MemberAffairsService;
exports.memberAffairsService = new MemberAffairsService();
