...
class AdminService {
    async getDashboardStats(_requester) {
        return admin_repository_1.adminRepository.getDashboardStats();
    }
    async getAllUsers(requester) {
        if (requester.role === 'SERVICE_MANAGER') {
            return admin_repository_1.adminRepository.getAllUsers(requester.serviceClassID);
        }
        return admin_repository_1.adminRepository.getAllUsers();
    }
    // ─── User Approval ──────────────────────────────────────────────
    async approveUser(adminId, targetId, ip) {
        const user = await admin_repository_1.adminRepository.findUserById(targetId);
        if (!user)
            throw new errors_1.NotFoundError('User not found');
        if (user.status === 'ACTIVE')
            throw new errors_1.BadRequestError('User is already active');
        const updated = await admin_repository_1.adminRepository.updateUser(targetId, { status: 'ACTIVE' });
        await admin_repository_1.adminRepository.logActivity({
            actorID: adminId, actionType: 'APPROVE_USER',
            targetUserID: targetId, description: `Approved user ${user.username}`, ipAddress: ip
        });
        return updated;
    }
    async rejectUser(adminId, targetId, ip) {
        const user = await admin_repository_1.adminRepository.findUserById(targetId);
        if (!user)
            throw new errors_1.NotFoundError('User not found');
        const updated = await admin_repository_1.adminRepository.updateUser(targetId, { status: 'SUSPENDED' });
        await admin_repository_1.adminRepository.logActivity({
    ...
    async suspendUser(adminId, requester, targetId, body, ip) {
        const user = await admin_repository_1.adminRepository.findUserById(targetId);
        if (!user)
            throw new errors_1.NotFoundError('User not found');
        if (user.id === adminId)
            throw new errors_1.BadRequestError('Cannot suspend yourself');
        if (requester.role === 'SERVICE_MANAGER' && user.serviceClassID !== requester.serviceClassID) {
            throw new errors_1.ForbiddenError('You can only suspend members of your own class');
        }
        const updated = await admin_repository_1.adminRepository.updateUser(targetId, { status: 'SUSPENDED' });
        await admin_repository_1.adminRepository.logActivity({
    ...
    async promoteRole(adminId, requester, targetId, body, ip) {
        const user = await admin_repository_1.adminRepository.findUserById(targetId);
        if (!user)
            throw new errors_1.NotFoundError('User not found');
        if (user.id === adminId)
            throw new errors_1.BadRequestError('Cannot change your own role');
        ...
        if (['SECRETARIAT_VICE', 'SECRETARIAT_SECRETARY'].includes(targetRole) && !canAssignSecretariatRoles) {
            throw new errors_1.ForbiddenError('Only the Secretariat Chairman can assign Secretariat roles');
        }
        ...
        if (targetRole === 'SERVICE_MANAGER' && !canAssignServiceManager) {
            throw new errors_1.ForbiddenError('Only the Secretariat Chairman or Vice Chairman can assign Service Manager roles');
        }
        // Teacher role assignment with Education Manager domain lock
        if (targetRole === 'TEACHER') {
            if (!canAssignTeacher) {
                throw new errors_1.ForbiddenError('You do not have permission to assign TEACHER roles');
            }
            if (requesterRole === 'SERVICE_MANAGER') {
                const requesterClass = await db_1.db.serviceClass.findUnique({ where: { id: requester.serviceClassID } });
                if (requesterClass?.class_name_amharic !== 'ትምርት ክፍል') {
                    throw new errors_1.ForbiddenError('Only the Education Manager can grant TEACHER roles');
                }
            }
        }
        ...
        if (requesterRole === 'SERVICE_MANAGER' &&
            ...
            throw new errors_1.ForbiddenError('Service Managers can only assign CLASS_LEADER or TEACHER roles');
        }
        // Business rule: SERVICE_MANAGER must be assigned to a specific class
        if (targetRole === 'SERVICE_MANAGER' && !body.serviceClassId) {
            throw new errors_1.BadRequestError('Service Manager role requires a service class assignment');
        }
        // Business rule: User must be a MEMBER to be promoted to CLASS_LEADER
        if (targetRole === 'CLASS_LEADER' && user.role !== 'MEMBER') {
            throw new errors_1.BadRequestError('User must be a MEMBER before being assigned CLASS_LEADER role');
        }
        try {
            const updateData = { system_role: targetRole };
            if (targetRole === 'SERVICE_MANAGER' && body.serviceClassId) {
                updateData.service_class_id = body.serviceClassId;
            }
            const updated = await admin_repository_1.adminRepository.updateUser(targetId, updateData);
            await admin_repository_1.adminRepository.logActivity({
            ...
            if (error.code === 'P2005') {
                throw new errors_1.BadRequestError(`Invalid role value: ${targetRole}. Please ensure the role is valid.`);
            }
            throw error;
        }
    }
    async changeUserClass(adminId, targetId, body, ip) {
        const user = await admin_repository_1.adminRepository.findUserById(targetId);
        if (!user)
            throw new errors_1.NotFoundError('User not found');
        ...
        const updated = await admin_repository_1.adminRepository.updateUser(targetId, { serviceClassID: body.serviceClassID });
        await admin_repository_1.adminRepository.logActivity({
    ...
    // ─── Leader Promotion ───────────────────────────────────────────
    async promoteLeader(adminId, targetId, body, ip) {
        const user = await admin_repository_1.adminRepository.findUserById(targetId);
        if (!user)
            throw new errors_1.NotFoundError('User not found');
        const cls = await db_1.db.serviceClass.findUnique({ where: { id: body.classID } });
        if (!cls)
            throw new errors_1.NotFoundError('Service class not found');
        const updatedUser = await admin_repository_1.adminRepository.updateUser(targetId, {
    ...
    async demoteLeader(adminId, targetId, ip) {
        const user = await admin_repository_1.adminRepository.findUserById(targetId);
        if (!user)
            throw new errors_1.NotFoundError('User not found');
        if (user.role !== 'SERVICE_MANAGER')
            throw new errors_1.BadRequestError('User is not a SERVICE_MANAGER');
        const updatedUser = await admin_repository_1.adminRepository.updateUser(targetId, {
            role: 'MEMBER',
        });
        await admin_repository_1.adminRepository.logActivity({
    ...
    async getSubClasses(requester) {
        if (requester.role === 'SERVICE_MANAGER') {
            return admin_repository_1.adminRepository.getSubClasses(requester.serviceClassID);
        }
        return admin_repository_1.adminRepository.getSubClasses();
    }
    async createSubClass(adminId, requester, body, ip) {
        if (!requester.serviceClassID)
            throw new errors_1.BadRequestError('You do not belong to a service class');
        const subClass = await admin_repository_1.adminRepository.createSubClass(requester.serviceClassID, body.name, 'PENDING_APPROVAL');
        await admin_repository_1.adminRepository.logActivity({
    ...
    async updateSubClassRoles(adminId, requester, subClassId, body, ip) {
        // Enforce same-class scope
        const subClass = await db_1.db.sub_classes.findUnique({ where: { id: subClassId } });
        if (!subClass)
            throw new errors_1.NotFoundError('Sub-class not found');
        if (requester.role === 'SERVICE_MANAGER' && subClass.parent_class_id !== requester.serviceClassID) {
            throw new errors_1.ForbiddenError('You can only modify sub-classes in your own department');
        }
        const updated = await admin_repository_1.adminRepository.updateSubClassRoles(subClassId, body);
        await admin_repository_1.adminRepository.logActivity({
    ...
    async getPendingSubClassApprovals() {
        return admin_repository_1.adminRepository.getPendingSubClassApprovals();
    }
    async approveSubClass(adminId, subClassId, ip) {
        const subClass = await db_1.db.sub_classes.findUnique({ where: { id: subClassId } });
        if (!subClass)
            throw new errors_1.NotFoundError('Sub-class not found');
        await admin_repository_1.adminRepository.approveSubClass(subClassId);
        await admin_repository_1.adminRepository.logActivity({
    ...
    async rejectSubClass(adminId, subClassId, ip) {
        const subClass = await db_1.db.sub_classes.findUnique({ where: { id: subClassId } });
        if (!subClass)
            throw new errors_1.NotFoundError('Sub-class not found');
        await admin_repository_1.adminRepository.rejectSubClass(subClassId);
        await admin_repository_1.adminRepository.logActivity({
    ...
    // ─── Office (ፅሕፈት ቤት) ──────────────────────────────────────────
    async getOfficeData() {
        return admin_repository_1.adminRepository.getOfficeData();
    }
    async getPendingOfficeRequests() {
        return admin_repository_1.adminRepository.getPendingOfficeRequests();
    }
    async approveOfficeRequest(adminId, targetId, ip) {
        const user = await admin_repository_1.adminRepository.findUserById(targetId);
        if (!user)
            throw new errors_1.NotFoundError('User not found');
        const officeClassId = await admin_repository_1.adminRepository.getOfficeClassId();
        if (!officeClassId)
            throw new Error('ፅሕፈት ቤት class not found in DB');
        const updated = await admin_repository_1.adminRepository.updateUser(targetId, {
    ...
    async disapproveOfficeRequest(adminId, targetId, ip) {
        const user = await admin_repository_1.adminRepository.findUserById(targetId);
        if (!user)
            throw new errors_1.NotFoundError('User not found');
        const unassignedClassId = await admin_repository_1.adminRepository.getUnassignedClassId();
        if (!unassignedClassId)
            throw new Error('Unassigned class not found in DB');
        const updated = await admin_repository_1.adminRepository.updateUser(targetId, {
    ...
    async assignRole(adminId, requester, body, ip) {
        if (requester.role !== 'SECRETARIAT_CHAIRMAN' && requester.role !== 'SUPER_ADMIN') {
            throw new errors_1.ForbiddenError('Only Chairman can assign executive roles');
        }
        const targetUser = await admin_repository_1.adminRepository.findUserById(body.targetUserId);
        if (!targetUser)
        ...
        if (!validRoles.includes(body.role)) {
            throw new errors_1.BadRequestError('Invalid role for assignment');
        }
        const updateData = { role: body.role };
        if (body.role === 'SERVICE_MANAGER' && body.serviceClassId) {
            updateData.serviceClassID = body.serviceClassId;
        }
        const updated = await admin_repository_1.adminRepository.updateUser(body.targetUserId, updateData);
        await admin_repository_1.adminRepository.logActivity({
    ...
    async revokeRole(adminId, requester, targetUserId, ip) {
        if (requester.role !== 'SECRETARIAT_CHAIRMAN' && requester.role !== 'SUPER_ADMIN') {
            throw new errors_1.ForbiddenError('Only Chairman can revoke executive roles');
        }
        const targetUser = await admin_repository_1.adminRepository.findUserById(targetUserId);
        if (!targetUser)
            throw new errors_1.NotFoundError('Target user not found');
        const updated = await admin_repository_1.adminRepository.updateUser(targetUserId, { role: 'MEMBER' });
        await admin_repository_1.adminRepository.logActivity({
    ...
    async transferChairman(currentChairmanId, targetUserId, ip) {
        const currentChairman = await admin_repository_1.adminRepository.findUserById(currentChairmanId);
        if (!currentChairman)
            throw new errors_1.NotFoundError('Current chairman not found');
        if (currentChairman.role !== 'SECRETARIAT_CHAIRMAN') {
            throw new errors_1.ForbiddenError('Only current Chairman can transfer chairmanship');
        }
        const targetUser = await admin_repository_1.adminRepository.findUserById(targetUserId);
        if (!targetUser)
            throw new errors_1.NotFoundError('Target user not found');
    ...
    async getAuditLogs(query) {
        ...
        const logs = await db_1.db.$queryRawUnsafe(`SELECT * FROM audit_logs
            WHERE $1::text IS NULL OR entity_type = $1
            AND $2::uuid IS NULL OR user_id = $2::uuid
            ORDER BY created_at DESC
            LIMIT $3 OFFSET $4`, entityType || null, userId || null, parseInt(limit), parseInt(offset));
        return logs;
    }
    // ─── Member Census ───────────────────────────────────────────────────
